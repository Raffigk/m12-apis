...
Director <- function(movie.data.frame) {
  movie.director <- movie.data.frame$Director
  movie.title <- movie.data.frame$Title
  movie.info <- paste(movie.title, movie.director)
  return(movie.info)
}
...
movie.list <- list('Finding Nemo', 'Toy Story', 'The Incredibles', 'Finding Dory')
